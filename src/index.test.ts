...
describe('cors-proxy worker', () => {
  ...
  describe('OPTIONS preflight', () => {
    ...
    it('returns 204 for private-pages.nsheaps.dev origin', async () => {
      const req = request('https://proxy.example.com/login/oauth/access_token', {
        method: 'OPTIONS',
        headers: { Origin: 'https://private-pages.nsheaps.dev' },
      });
      const res = await worker.fetch(req, env);
      expect(res.status).toBe(204);
      expect(res.headers.get('Access-Control-Allow-Origin')).toBe('https://private-pages.nsheaps.dev');
    });

    it('returns 204 for cept.nsheaps.dev origin', async () => {
      const req = request('https://proxy.example.com/login/oauth/access_token', {
        method: 'OPTIONS',
        headers: { Origin: 'https://cept.nsheaps.dev' },
      });
      const res = await worker.fetch(req, env);
      expect(res.status).toBe(204);
    });
